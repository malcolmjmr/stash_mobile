...
class WebManager extends ChangeNotifier {
  ...
  setResource(Resource newResource) async {
    resource = newResource;
    if (resource.url != null) {
      await controller?.loadUrl(urlRequest: URLRequest(url: Uri.parse(resource.url!)));
    }
  ...
  Future<String?> getFaviconUrl() async {
    ...
    return favIcons != null && favIcons.isNotEmpty ? favIcons.first.url.toString() : null;
  }
  ...
  onWebsiteLoadStart(BuildContext context, Uri? uri) async {
    // check if resource is saved and increment view count
    if (resource.url == uri.toString()) {
